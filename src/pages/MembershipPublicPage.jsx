...
const MembershipPublicPage = () => {
  ...
  const parseExpiry = (expiry) => {
    ...
    return { month: mm?.trim(), year: yy?.trim()?.length === 2 ? `20${yy.trim()}` : yy?.trim() };
  };

  const validate = () => {
    if (!PUBLIC_ORG_ID) return 'El registro público no está configurado. Falta VITE_PUBLIC_ORG_ID.';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc('public_signup_membership', {
        p_org_id: PUBLIC_ORG_ID,
        p_customer: {
      ...
      setCreated(data);
      setStep('success');
      toast({ title: 'Membresía registrada', description: `Plan ID: ${data.membership.plan_id}` });
    } catch (err) {
      console.error(err);
  ...
  const renderPlans = () => (
    <div className="max-w-5xl mx-auto space-y-8">
      <div className="text-center space-y-3">
        ...
        <p className="text-lg text-slate-600">Cuida de tu salud y la de tu familia con beneficios todos los meses.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {Object.values(PLANS).map((p) => (
          <Card
            key={p.key}
            className={`cursor-pointer transition-all hover:shadow-xl ${selectedPlanKey === p.key ? 'ring-2 ring-blue-500' : ''}`}
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-4">
            <label className={`flex-1 flex items-center gap-3 p-3 border rounded-lg cursor-pointer ${form.paymentMethod === 'card' ? 'bg-blue-50 border-blue-300' : 'hover:bg-slate-50'}`}>
              <input type="radio" name="paymentMethod" value="card" checked={form.paymentMethod === 'card'} onChange={(e) => updateField('paymentMethod', e.target.value)} className="w-4 h-4" />
            ...
            <label className={`flex-1 flex items-center gap-3 p-3 border rounded-lg cursor-pointer ${form.paymentMethod === 'cash' ? 'bg-amber-50 border-amber-300' : 'hover:bg-slate-50'}`}>
              <input type="radio" name="paymentMethod" value="cash" checked={form.paymentMethod === 'cash'} onChange={(e) => updateField('paymentMethod', e.target.value)} className="w-4 h-4" />
          ...
          <div className="space-y-2">
            ...
            {!isOpenPayConfigured() && (
              <div className="p-3 bg-yellow-50 text-yellow-800 text-sm rounded-lg">
                El procesador de pagos no está configurado. Puedes registrarte y agregar tus credenciales de pago después.
              </div>
            )}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <Label>Número de tarjeta</Label>
                <Input value={form.cardNumber} onChange={(e) => updateField('cardNumber', e.target.value)} placeholder="0000 0000 0000 0000" maxLength={19} />
              </div>
              <div className="md:col-span-2">
              ...
              <div>
                ...
                <Input value={form.expiry} onChange={(e) => updateField('expiry', e.target.value)} placeholder="MM/AA" maxLength={5} />
              </div>
              <div>
                <Label>CVV</Label>
                <Input type="password" value={form.cvv} onChange={(e) => updateField('cvv', e.target.value)} placeholder="123" maxLength={4} />
              </div>
            </div>
          </div>
        )}
  ...
  const renderSuccess = () => (
    <div className="max-w-xl mx-auto text-center space-y-6">
      ...
      <div className="bg-slate-50 rounded-xl p-6 space-y-2 text-left">
        ...
        <p className="text-2xl font-mono font-bold text-slate-900">{created?.membership?.plan_id}</p>
        <p className="text-sm text-slate-500">Titular</p>
        <p className="font-medium">{created?.customer?.full_name}</p>
        <p className="text-sm text-slate-500">Renovación</p>
        <p className="font-medium">{created?.membership?.next_renewal_date}</p>
        <p className="text-sm text-slate-500">Monto mensual</p>
        <p className="font-medium">${Number(created?.membership?.monthly_amount).toFixed(2)} MXN</p>
      </div>
